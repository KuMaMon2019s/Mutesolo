...
```
rg "func handle" internal/webapp/server.go     # HTTP handlers (line 91-1601)
...
- **API project actions:** `internal/webapp/server.go:662` (handleProjectActions)
- **Board update:** `internal/webapp/server.go:1228` (handleBoardUpdate, routed via handleProjectActions)
- **Project delete:** `internal/webapp/server.go:616` (handleProjectDelete, routed via handleProjectActions)
- **Branch management:** `internal/webapp/server.go:712` (handleBranches), `L741` (handleBranchDetail)
- **Requirement CRUD:** `internal/webapp/server.go:762` (handleRequirements), `L801` (handleRequirementDetail), `L812` (handleRequirementUpdate), `L1047` (handleRequirementDelete), `L1093` (handleRequirementGet)
- **Requirement by ID:** `internal/webapp/server.go:1093` (handleRequirementGet — standalone /api/requirement endpoint)
- **Project image:** `internal/webapp/server.go:578` (handleProjectImage), `L543` (uploadCoverForProject), `L569` (picSumCoverURL)
...
- **Handler:** `internal/webapp/server.go:1122` (handlePrompt)
- **Send prompt:** `internal/webapp/server.go:1192` (handleSendPrompt)
- **Generate prompt:** `internal/webapp/server.go:1261` (handleGeneratePrompt)
- **Save prompt to requirement:** `internal/webapp/server.go:1175` (savePromptToRequirement)
...
- **LLM test:** `internal/webapp/server.go:1406` (handleLLMTest), `internal/webapp/llm.go:111` (TestOpenCodeConnection)
...
- **Base64 image processing:** `internal/webapp/server.go:874` (processBase64Images)
- **Attachment parsing:** `internal/webapp/server.go:956` (parseAttachments), `L968` (marshalAttachments)
- **Editor → HTML:** `internal/webapp/server.go:982` (editorContentToHTMLDescription)
- **Blocks → HTML:** `internal/webapp/server.go:1002` (blocksToHTML), `L1026` (docToHTML)
- **Text extraction:** `internal/webapp/server.go:1035` (extractTextFromContent), `L1360` (extractPlainText), `L1388` (collectText)
- **Image detection:** `internal/webapp/server.go:1352` (isImageAttachment)
...
- **Workload API:** `internal/webapp/server.go:1452` (handleAgentWorkload) — task counts per agent
- **Agent tasks API:** `internal/webapp/server.go:1518` (handleAgentTasks) — all tasks for a given agent
...
- **Stats:** `internal/webapp/server.go:1434` (handleStats), `webapps/control-console/src/api/projects.ts:77` (fetchStats)
...
- **Asset ref dispatch:** `internal/webapp/server.go:1633-1727` (Server.saveAssetRef, getAssetRefsByProject, getAssetRefsByRequirement, deleteAssetRefsAndMinIO, deleteAssetRefsByProject, deleteAssetRefsByRequirement — dual-backend dispatch)
- **Server helpers:** `internal/webapp/server.go:1617` (runGit), `L1745` (writeJSON), `L1750` (writeError)
- **CORS middleware:** `internal/webapp/server.go:1759` (corsMiddleware)
...
- **Send helper:** `internal/webapp/discord_sync.go:46` (sendWebhook)
...
- **Handler:** `internal/webapp/server.go:1601` (handleGitHubPush)
...
- **Workload handler:** `internal/webapp/server.go:1452` (handleAgentWorkload — aggregate counts across all projects)
- **Agent tasks handler:** `internal/webapp/server.go:1518` (handleAgentTasks — tasks filtered by agent+project)
...
| GET | /api/stats | handleStats | server.go:1434 |
| GET | /api/agent-workload | handleAgentWorkload | server.go:1452 |
| GET | /api/agent-tasks | handleAgentTasks | server.go:1518 |
...
| POST | /api/llm/test | handleLLMTest | server.go:1406 |
| GET/POST | /api/projects | handleProjects | server.go:506 |
| GET/PUT/DELETE | /api/projects/{id} | handleProjectActions | server.go:662 |
| GET/PUT | /api/requirement | handleRequirementGet | server.go:1093 |
| POST | /api/generate-prompt | handleGeneratePrompt | server.go:1261 |
| POST | /api/github/push | handleGitHubPush | server.go:1601 |
...
| File | Why |
|------|-----|
| `internal/webapp/server.go` | All HTTP handlers, ~1772 lines |
