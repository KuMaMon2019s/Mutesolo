...
> Generated by Atris | Last updated: 2026-07-20
...
rg "func main" sync-discord/main.go            # Discord sync queue processor
...
- **Board:** `webapps/control-console/src/pages/Board.tsx:25` (Kanban board)
...
- **SSE session:** `sync-discord/main.go:373` (startSSESession), `L433` (extractSessionID), `L449` (closeSSESession)
- **MCP calls:** `sync-discord/main.go:580` (callMCP)
- **Discord helpers:** `sync-discord/main.go:689` (findOrCreateCategory), `L747` (createTextChannel), `L784` (sendWelcomeMessage), `L803` (upsertMemberPerms), `L817` (upsertRolePerms)
- **Queue reading:** `sync-discord/main.go:838` (readQueue)
- **Discord user resolution:** `sync-discord/main.go:520` (extractWidgetURL), `L553` (resolveDiscordUserID)
- **Self ID:** `sync-discord/main.go:460` (loadSelfDiscordID)
- **Config fetch:** `sync-discord/main.go:499` (fetchMutesoloConfig)
...
| `fetchDiscordGuildMembers` | `projects.ts` | 69 | Full guild member list via bot token |
| `fetchAIAgentScreenshotMembers` | `projects.ts` | 73 | Widget screenshot |
| `fetchMembers` | `projects.ts` | 77 | Persistent members |
| `fetchStats` | `projects.ts` | 81 | Member stats (project/branch filtered) |
| `fetchAgentWorkload` | `projects.ts` | 109 | Agent workload counts |
| `fetchAgentTasks` | `projects.ts` | 113 | Tasks per agent |
| `fetchClawHubSkills` | `projects.ts` | 120 | Skill list |
| `fetchClawHubSkillDetail` | `projects.ts` | 152 | Skill detail |
| `installSkill` | `projects.ts` | 156 | Install skill |
| `fetchPluginRuntimes` | `projects.ts` | 163 | Runtime list |
